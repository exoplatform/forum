...
/**
  ...
  version: 2.5.2
 */
KSBrowser.prototype.detectBrowser = function() {
...
KSBrowser.prototype.managerResize = function() {
  if(eXo.ks.Browser.currheight != document.documentElement.clientHeight) {
    clearTimeout(eXo.ks.Browser.breakStream) ;
    eXo.ks.Browser.breakStream = setTimeout(eXo.ks.Browser.onResize, 100) ;
  }
  eXo.ks.Browser.currheight = document.documentElement.clientHeight;
}

KSBrowser.prototype.initCommon = function() {
...
KSBrowser.prototype.initIE = function() {
  this.browserType = "ie" ;
...
KSBrowser.prototype.initMozilla = function() {
...
KSBrowser.prototype.initSafari = function() {
...
KSBrowser.prototype.initOpera = function() {
...
KSBrowser.prototype.isIE6 = function() {
  var agent = navigator.userAgent ;
  return (agent.indexOf("MSIE 6") >=0);
} ;

KSBrowser.prototype.isIE7 = function() {
  var agent = navigator.userAgent ;
  return (agent.indexOf("MSIE 7") >=0);
} ;

KSBrowser.prototype.isFF = function() {
  return this.gecko;
} ;

KSBrowser.prototype.isFF2 = function() {
  return (navigator.userAgent.indexOf("Firefox/2") >= 0);
} ;

KSBrowser.prototype.isFF3 = function() {
  return (navigator.userAgent.indexOf("Firefox/3") >= 0);
} ;

KSBrowser.prototype.findMouseXInClient = function(e) {
  if (!e) e = window.event ;
  return e.clientX ;
} ;

KSBrowser.prototype.findMouseYInClient = function(e) {
  if (!e) e = window.event ;
  return e.clientY ;
} ;
/**
 ...
 */
KSBrowser.prototype.addOnLoadCallback = function(id, method) {
 ...
 * and clean the array
 */
KSBrowser.prototype.onLoad = function() {
  var callback = eXo.ks.Browser.onLoadCallback ;
 ...
 */
KSBrowser.prototype.addOnResizeCallback = function(id, method) {
...
KSBrowser.prototype.onResize = function(event) {
  var callback = eXo.ks.Browser.onResizeCallback ;
 for(var name in callback.properties) {
   var method = callback.get(name) ;
 ...
 */
KSBrowser.prototype.addOnScrollCallback = function(id, method) {
  this.onScrollCallback.put(id, method) ;
} ;
/**
 * Calls the functions in the onScrollCallback array, if they exist
 */
KSBrowser.prototype.onScroll = function(event) {
  var callback = eXo.ks.Browser.onScrollCallback ;
  for(var name in callback.properties) {
    var method = callback.get(name) ;
    try {
      if (typeof(method) == "function") method(event) ;
...
KSBrowser.prototype.getBrowserType = function() {  
  return this.browserType ;
} ;
/**
 * Returns the horizontal position of an object relative to the window
 */
KSBrowser.prototype.findPosX = function(obj, isRTL) {
  var curleft = 0;
  var tmpObj = obj ;
  while (tmpObj) {
 ...
 */
KSBrowser.prototype.findPosY = function(obj) {
  var curtop = 0 ;
  while (obj) {
 ...
 */
KSBrowser.prototype.findPosXInContainer = function(obj, container, isRTL) {
  var objX = eXo.ks.Browser.findPosX(obj, isRTL) ;
  var containerX = eXo.ks.Browser.findPosX(container, isRTL) ;
 ...
 */
KSBrowser.prototype.findPosYInContainer = function(obj, container) {
  var objY = eXo.ks.Browser.findPosY(obj) ;
  var containerY = eXo.ks.Browser.findPosY(container) ;
 ...
 */
KSBrowser.prototype.findMouseXInPage = function(e) {
  var posx = -1 ;
  if (!e) e = window.event ;
  if (e.pageX || e.pageY) {
    posx = e.pageX ;
 ...
 */
KSBrowser.prototype.findMouseYInPage = function(e) {
  var posy = -1 ;
  if (!e) e = window.event ;
  if (e.pageY) {
    posy = e.pageY ;
 ...
 */
KSBrowser.prototype.findMouseRelativeX = function(object, e, isRTL) {
  var posx = -1 ;
  var posXObject = eXo.ks.Browser.findPosX(object,isRTL) ;
 ...
 */
KSBrowser.prototype.findMouseRelativeY = function(object, e) {
  var posy = -1 ;
  var posYObject = eXo.ks.Browser.findPosY(object) ;
  if (!e) e = window.event ;
  if (e.pageY) {
 ...
 */
KSBrowser.prototype.setPositionInContainer = function(container, component, posX, posY) {
  var offsetX = component.offsetLeft ;
  var offsetY = component.offsetTop ;

  var posXInContainer = eXo.ks.Browser.findPosXInContainer(component, container) ;
  var posYInContainer = eXo.ks.Browser.findPosYInContainer(component, container) ;
 ...
 * Set Cookie
 */
KSBrowser.prototype.setCookie = function(name,value,expiredays) {
  var exdate = new Date() ;
 ...
 * Get Cookie
 */
KSBrowser.prototype.getCookie = function(name) {
...
KSBrowser.prototype.isDesktop = function() {
 ...
 */
KSBrowser.prototype.getHeightOfFreeSpace = function() {
 ...
 */
KSBrowser.prototype.fillUpFreeSpace = function(elemt) {
  if(typeof(elemt) == "string")
    elemt = document.getElementById(elemt);
	
  var height = eXo.ks.Browser.getHeightOfFreeSpace();
