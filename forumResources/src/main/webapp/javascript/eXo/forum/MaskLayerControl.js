var MaskLayerControl = {
  ...
  getContainerNode : function () {
    ...
    if (!containerNode.exists()) {
      ...
      containerNode.attr('id', 'UIPictutreContainer')
                   .attr('style', 'position:absolute; top:0px; width:100%; height:100%; text-align:center')
                   .attr('title', 'Click to close')
                   .on('click', MaskLayerControl.hidePicture);
  ...
  showPicture : function(node) {
    ...
    if (String(src).length > 0) {
      ...
      var maskNode = eXo.core.UIMaskLayer.createMask('UIPortalApplication', containerNode[0], 30, 'CENTER');
      this.scrollHandler();
...
_module.MaskLayerControl = MaskLayerControl;
